...
Suppose $$Y$$ is a random variable where $$E[Y] = \mu$$ and $$var[Y] = \sigma^2$$. Let $$X = g(Y)$$ for some differentiable function $$g$$. Then we can derive from the Taylor series expansion $$g(Y) = g(\mu) + g'(\mu)(Y - \mu) + C$$. 
