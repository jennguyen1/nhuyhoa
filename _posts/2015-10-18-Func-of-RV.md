...
Suppose $$Y$$ is a random variable where $$E[Y] = \mu$$ and $$var[Y] = \sigma^2$$. Let $$X = g(Y)$$ for some differentiable function $$g$$, where $$g'(\mu)$$ exists and is non-zero. Then we can derive from the Taylor series expansion $$g(Y) = g(\mu) + g'(\mu)(Y - \mu) + C$$. 

$$E(X) \approx g(\mu)$$

$$Var(X) \approx [g'(\mu)]^2 Var(Y) = [g'(\mu)]^2 \sigma^2$$

Using these moments, we may also apply the CLT to obtain the convergence distribution of the random variable. 

**Example:**

Let $$Y \sim Bin(n, p)$$ and $$\hat{\theta} = \log \left( \frac{Y}{n - Y} \right)$$

Then $$g(y) = \log \left( \frac{y}{n - y} \right) = \log(y) - \log(n - y)$$ <br>
And $$\frac{\partial}{\partial y}g(y) = \frac{1}{y} + \frac{1}{n - y} = \frac{n}{y(n - y)}$$

Since $$E[Y] = np$$, $$g'(np) = \frac{n}{np(n - np)} = \frac{1}{np(1 - p)}$$ <br>
And $$Var(\hat{\theta}) = \left( \frac{1}{np(1-p)} \right)^2 np(1-p) = \frac{1}{np(1-p)}$$
