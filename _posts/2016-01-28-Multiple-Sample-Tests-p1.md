...
These effects can be assessed by generating an interaction plot. By assessing the trends and parallelism of lines, it can be determined whether there may be evidence of an interaction effect. These effects can also be assed with a contrast.
...
Models with subsampling are essentially the same as models that average those subsamples for each experimental unit. Thus even though the $$SS$$ and $$MS$$ for these models are different, the $$F$$ statistic, degrees of freedom, and p-value for effects are all the same. If subsamples are not averaged, they need to be accounted for.
...
To fit repeated measures, specify the covariance structure for $$\epsilon$$, specifically the correlations between measurements within an individual. To determine the best covariance structure, compare model using model statistics such as AIC or BIC. 
