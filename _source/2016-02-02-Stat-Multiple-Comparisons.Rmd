...
set.seed(5)
null <- plyr::raply(500, t.test(rnorm(100), rnorm(100))$p.value)
non_null <- plyr::raply(250, t.test(rnorm(100), rnorm(100, 0.25))$p.value)
...
```{r, echo = FALSE}
set.seed(3)
null <- plyr::raply(800, t.test(rnorm(150), rnorm(150))$p.value)
non_null <- plyr::raply(800, t.test(rnorm(150), rnorm(150, 0.4))$p.value)

plot_data <- data.frame(p.vals = c(null, non_null), type = c(rep("null", length(null)), rep("non-null", length(non_null))))
plot_data$type <- reorder(plot_data$type, ifelse(plot_data$type == "non-null", 1, 2))
