...
# extract the 25th-30th students in the data set
data %>% slice(25:30) %>% nhuyhoa_df_print(head = 6)
```
</div><p></p>
...
data %>% dplyr::select(posttest_score, posttest_grade, d_gender, posttest_subject) %>% nhuyhoa_df_print()
```
</div><p></p>
...
data %>% dplyr::select(-posttest_score, -posttest_grade, -posttest_subject) %>% nhuyhoa_df_print()
```
</div><p></p>
...
var_pos <- 26:29 
data %>% dplyr::select(var_pos) %>% nhuyhoa_df_print()
```
</div><p></p>
...
var_names <- c("d_black", "d_hispanic", "d_asian", "d_native")
data %>% dplyr::select(one_of(var_names)) %>% nhuyhoa_df_print()
```
</div><p></p>
...
data %>% dplyr::select(matches("pretest_\\d+_score")) %>% nhuyhoa_df_print()
```
</div><p></p>
...
data %>% dplyr::select(starts_with("posttest")) %>% nhuyhoa_df_print()
```
</div><p></p>
...
data %>% dplyr::select(ends_with("_score")) %>% nhuyhoa_df_print()
```
</div><p></p>
...
data %>% dplyr::select(student_id, matches("pretest_\\d+_score"), d_gender, starts_with("d_")) %>% nhuyhoa_df_print()
```
</div><p></p>
...
no_good <- paste0("pretest_10_", c("score", "version", "grade"))
data %>% dplyr::select(student_id, starts_with("pretest_"), -one_of(no_good)) %>% nhuyhoa_df_print()
```
</div><p></p>
...
{% endhighlight %}
<div class = "dftab">
```{r dplyr select ex12, echo = FALSE}
order <- sample(1:ncol(data))
unorganized_data <- dplyr::select(data, order)
unorganized_data %>% dplyr::select(student_id, matches("posttest"), matches("pretest"), starts_with("d_")) %>% nhuyhoa_df_print()
```
</div><p></p>
