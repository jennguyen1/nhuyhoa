...
order <- sample(1:ncol(data))
unorganized_data <- dplyr::select(data, order)
unorganized_data %>% dplyr::select(student_id, matches("posttest"), matches("pretest"), d_gender, d_gifted, everything()) %>% nhuyhoa_df_print()
```
</div><p></p>
...
unorganized_data %>% dplyr::select(-starts_with("d_"), everything()) %>% nhuyhoa_df_print()
```
</div><p></p>
