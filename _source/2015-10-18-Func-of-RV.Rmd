...
# Delta Method
If the variance of a random variable cannot be computed directly, one can use the delta method. 

Suppose $$Y$$ is a random variable where $$E[Y] = \mu$$ and $$var[Y] = \sigma^2$$. Let $$X = g(Y)$$ for some differentiable function $$g$$. Then 
...
Let $$Y$$ ~ $$Bin(n, p)$$ and $$\hat{\theta} = \log \left( \frac{Y}{n - Y} \right)$$

Then $$g(y) = \log \left( \frac{y}{n - y} \right) = \log(y) - \log(n - y)$$
And $$\frac{\partial}{\partial y}g(y) = \frac{1}{y} + \frac{1}{n - y} = \frac{n}{y(n - y)}$$

Since $$E[Y] = np$$, $$g'(np) = \frac{n}{np(n - np)} = \frac{1}{np(1 - p)}$$
