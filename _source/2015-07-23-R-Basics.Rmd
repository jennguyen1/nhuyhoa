...
list.files(DIR, full.names = TRUE) # obtain file paths for all files in DIR
```

Opening files can be done with any of the following functions
```{r, eval = FALSE}
read.csv()
data.table::fread()
readxl::excel_sheets()
...
* Connecting to a SQL server (done through packages)
