...
set.seed(5)
null <- raply(500, t.test(rnorm(100), rnorm(100))$p.value)
non_null <- raply(250, t.test(rnorm(100), rnorm(100, 0.25))$p.value)
plot_data <- data.frame(i = 1:750, p.vals = sort(c(null, non_null)))
g1 <- ggplot() + 
  geom_point(data = plot_data, aes(i, p.vals)) +
  geom_line(data = NULL, aes(1:750, 1:750/750*0.05)) 

g2 <- g1 + xlim(0, 40) + ylim(0, 0.005) + ggtitle("Close Up")
...
plot_data <- mutate(plot_data, cutoff = i / 750 * 0.05)

cutoff <- plot_data %>% 
  subset(p.vals < cutoff) %>% 
  arrange(desc(p.vals)) %>% 
  .$p.val %>% .[1]

q_values <- p.adjust(plot_data$p.vals, method = "fdr")
ggplot() +
  geom_point(data = NULL, aes(plot_data$p.vals, q_values)) +
  geom_hline(yintercept = 0.05) +
  geom_vline(xintercept = cutoff) + 
  xlab("p-values") + ylab("q-values (FDR)") + 
...
set.seed(5)
null <- raply(500, t.test(rnorm(100), rnorm(100))$p.value)
non_null <- raply(500, t.test(rnorm(100), rnorm(100, 0.4))$p.value)

plot_data <- data.frame(p.vals = c(null, non_null), type = c(rep("null", length(null)), rep("non-null", length(non_null))))
plot_data$type <- reorder(plot_data$type, ifelse(plot_data$type == "null", 1, 2))

ggplot(data = plot_data, aes(x = p.vals, fill = type)) + 
  geom_histogram(binwidth = .05, alpha = 0.75, color = "black") +
  geom_vline(xintercept = 0.5, color = "black", size = 1, linetype = 2) +
  scale_fill_manual(values = c("non-null" = "#0066ff", "null" = "#00cc00")) +
  xlab("p-values") + theme(legend.position = "bottom")
```
